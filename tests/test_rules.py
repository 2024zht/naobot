...
def test_reply_rules(message, expected):
    assert reply_for_text(message) == expected
...
def test_help_lists_fraud_keyword_commands():
    assert "/添加违规" in HELP_TEXT
    assert "/违规词列表" in HELP_TEXT
    assert "/删除违规词" in HELP_TEXT

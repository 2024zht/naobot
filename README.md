...
- 多人复读：同一群 30 秒内有 3 名不同成员发送完全相同的普通文本时，机器人也原样补一句
- 实验室问答：帮助菜单展示 `data/lab_faq.json` 中的问题，成员发送 `@机器人 完整问题` 可直接查看答案
- `@机器人 关键词`：持久化关键词回复
...
- 文字规则、图片和视频 OCR、二维码、QQ 名片和重要通知防护
- 违规累计 3 次自动踢出
...
参考 `examples/lab_faq.example.json` 创建 `data/lab_faq.json`，使用 UTF-8 JSON 对象保存问题和答案：
...
保存后无需重启机器人。`@nao 帮助` 会追加问题索引，成员发送完整问题即可获得答案。JSON 临时写坏时继续使用上一份有效内容，修正文件后会自动重新加载。
